...
/* 单文件 */
func (u Upload) File(file *multipart.FileHeader, params map[string]interface{}) string {
	...
	src, err := file.Open()
	if err != nil {
		fmt.Println("[Upload] Mkdir:", "打开文件失败!")
		return ""
	}
	defer src.Close()
	if !(&FileEo{}).Upload(src, param["path"].(string)+param["filename"].(string)) {
...
func (Upload) GetFileName() string {
	...
	return d + n + randA.String() + randB.String()
}
